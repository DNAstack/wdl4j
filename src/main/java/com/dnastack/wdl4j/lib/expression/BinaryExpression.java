...
public class BinaryExpression extends Expression {
    ...
    private Type typeCheckNumeric(CoercionOptions options, Type lhs, Type rhs) throws TypeCoercionException {
        Type floatType = FloatType.getType();
        Type intType = IntType.getType();

        if (!lhs.isCoercibleTo(options, floatType) && !lhs.isCoercibleTo(options, intType)) {
        ...
        if ((!rhs.isCoercibleTo(options, floatType) && !rhs.isCoercibleTo(options, intType))) {
        ...
        if (lhs.equals(intType) || lhs.equals(floatType)) {
            return lhs;
        } else if (lhs.isCoercibleTo(options, intType)) {
            return intType;
        } else {
            return floatType;
        }
    ...
    private Type typeCheckNumericOrString(CoercionOptions options, Type lhs, Type rhs) throws TypeCoercionException {

        Type targetType;
        if (lhs.equals(StringType.getType()) || lhs.equals(FloatType.getType()) || lhs.equals(IntType.getType())) {
            targetType = lhs;
        } else if (lhs.isCoercibleTo(options, FloatType.getType())) {
            targetType = FloatType.getType();
        } else if (lhs.isCoercibleTo(options, IntType.getType())) {
            targetType = IntType.getType();
        } else if (lhs.isCoercibleTo(options, StringType.getType())) {
            targetType = StringType.getType();
        } else {
            throw new TypeCoercionException("Illegal Type Coercion. Left hand side of binary operation " + operation.name() + " cannot be cast to Float, Int or String type");
        }

        if (!rhs.isCoercibleTo(options, targetType)) {
            throw new TypeCoercionException("Illegal Type Coercion. Right hand side of binary operation " + operation.name() + " cannot be cast to " + targetType
                    .getTypeName());
        }
    ...
    public Type typeCheck(WdlElement target, Namespace namespace) throws WdlValidationError {
        ...
        if (operation.equals(BinaryOperation.DIVIDE) || operation.equals(BinaryOperation.MULTIPLY) || operation.equals(
            ...
            return typeCheckNumeric(namespace.getCoercionOptions(), leftHandType, rightHandType);
        } else if (operation.equals(BinaryOperation.LOGICAL_AND) || operation.equals(BinaryOperation.LOGICAL_OR)) {
            return typeCheckBoolean(namespace.getCoercionOptions(), leftHandType, rightHandType);
            ...
            if (leftHandType.isCoercibleTo(namespace.getCoercionOptions(), StringType.getType())) {
